...
all-specs : all
	@for spec in specs/*; do if [ -f $$spec/Makefile -a $$spec != 'specs/template' ]; then echo "\nBuilding $$spec" && make -s -C $$spec/ all; fi; done
...
stabilize-issues-all-specs : stabilize-issues
	@for spec in specs/*; do if [ -f $$spec/Makefile -a $$spec != 'specs/template' ]; then make -s -C $$spec/ stabilize-issues; fi; done
